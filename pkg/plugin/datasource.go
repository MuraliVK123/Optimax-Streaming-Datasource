...
func (d *Datasource) query(ctx context.Context, pCtx backend.PluginContext, query backend.DataQuery) (backend.DataResponse, error) {
	...
	if len(query.JSON) > 0 {
		...
		case map[string]interface{}:
			...
			//cookieValue = data["cookieValue"].(string);
	...
	//return backend.DataResponse{}, fmt.Errorf("%w: expected 200 response, got %d", cookieValue1, "scale")

	// cookieName := d.settings.JSONData.Get("cookieName").MustString("")
	// cookieValue := d.settings.JSONData.Get("cookieValue").MustString("")

	var cookies interface{}
	err := json.Unmarshal(d.settings.JSONData, &cookies)
	if err != nil{
		fmt.Errorf("new request with context: %w", err)
	}else{
		switch data:= cookies.(type) {
		case map[string]interface{}:
			cookieName = data["cookieName"].(string) 
			cookieValue = data["cookieValue"].(string);
		}
	}
	...
	if httpResp.StatusCode != http.StatusOK {
		...
		//return backend.DataResponse{}, fmt.Errorf("%w: expected 200 responses"+ cookieValue + " got %d", errRemoteResponse, cookieName)
	}
...
func (d *Datasource) CheckHealth(ctx context.Context, _ *backend.CheckHealthRequest) (*backend.CheckHealthResult, error) {
	...
	return &backend.CheckHealthResult{
		...
		Message: "Data source is working Fine",
	}, nil
}
